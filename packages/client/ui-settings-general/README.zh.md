...
设置外壳、无特定功能归属文案与持久化产品引导 namespace。它以账号行触发器、账号菜单和模态设置面板占用 `sidebar.settings`，把 `settings.section` 账本投影成导航、把 `settings.onboarding` 账本投影成每次只挂载一个步骤的引导流程，并在设置页面上注册所有不属于单一功能的内容：触发器、标题栏与关闭控件内容、本地配置文件操作，「账户」分区、「通用」分区及其 `settings.general.item` slot，以及 `settings` 字典。「账户」（`id: account`，`order: -10`）是本机游客资料页：CORKS 标志、已存昵称或本地化「游客」、「本地游客」，以及资料仅保存在这台设备。可选的 `window.dshDesktop.setGuestDisplayName`、`clearGuestProfile` 和 `logout` 分别提供改名、确认后清除资料、退出登录；空白改名会清掉已存昵称，行回退为「游客」。浏览器标签页渲染该页但不提供这些操作。保存或清除会派发同一文档内的 `dsh-guest-display-name-changed` 事件。底部触发器是通栏账号条（圆形 CORKS 标志加本地化「游客」，或已存昵称）。点击后打开 portal 菜单：「设置」打开既有模态并落在「通用」；「外观」仅在挂载 `ui-theme` 时写入主题偏好，否则省略；「帮助」打开本机说明；仅当 `window.dshDesktop.logout` 与 `checkUpdates` 都存在时才渲染「检查更新」和「退出登录」。浏览器标签页保留「游客」并隐藏这两项。设置齿轮触发器不再直接打开对话框。它渲染进的那些 slot 类型归 ui-settings——设置领域底座——所有；只有外壳自身的契约类型放在这里，因为它们引用 ui-sidebar 的 slot 类型，而底座不得依赖任何 `ui-*` 包。归具体功能所有的行（「权限」、「语言」、「外观」）、分区（「模型」）和条件式首次使用引导步骤仍由各自的功能包提供。
