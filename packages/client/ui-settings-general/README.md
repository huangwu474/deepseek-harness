...
Settings shell, ownerless copy, and durable product-onboarding namespace. It occupies `sidebar.settings` with the account-row trigger, account menu, and modal settings panel, projects the `settings.section` ledger into the navigation and the `settings.onboarding` ledger into one mounted step at a time, and registers everything on the Settings pages that belongs to no single feature — the trigger/header/close chrome content, the local configuration-file action, the Account section, the General section and its `settings.general.item` slot, and the `settings` dictionaries. Account (`id: account`, `order: -10`) is the local guest profile page: CORKS mark, stored nickname or localized Guest, Local guest, and that the profile stays on this device. Optional `window.dshDesktop.setGuestDisplayName`, `clearGuestProfile`, and `logout` expose rename, clear-after-confirm, and logout; a blank rename clears the stored name and the row falls back to Guest. Browser tabs render the page without those actions. Saving or clearing dispatches a same-document `dsh-guest-display-name-changed` event. The foot trigger is a full-width account bar (circular CORKS mark plus localized Guest, or the stored nickname). Clicking it opens a portal menu: Settings opens the existing modal and lands on General; Appearance writes the theme preference when `ui-theme` is mounted and is omitted otherwise; Help shows a local notice; Check for updates and Log out render only when `window.dshDesktop.logout` and `checkUpdates` both exist. Browser tabs keep Guest and hide those two items. The settings-gear trigger does not open the dialog directly. The slot types it renders into belong to ui-settings, the settings domain base; only the shell's own contract types live here, because they reference ui-sidebar's slot type and the base layer must depend on no `ui-*` package. Feature-owned rows (Permission, Language, Appearance), sections (Models), and conditional onboarding steps stay with their feature packages.
